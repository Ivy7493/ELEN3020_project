...
def FetchGuestSamples(searchField):
    ...
    cols = ('Sample ID', 'Box ID', 'Box X', 'Box Y', 'Box Z', 'Sample Type', 'Origin Country', 'Collection Date', 'Entry Date', 'Sample History', 'Subject Age', 'Tube Rating', 'Collection Title', 'Donor Phone', 'Authorised Phone', 'Return Type', 'Return Date', 'Test Result', 'Phenotype Value', 'Disease State')
    tree = ttk.Treeview(window_Samples, columns=cols, show='headings')
    for col in cols:
        tree.heading(col, text=col)
    tree.grid(row=2, column=0, columnspan=20)

    c.execute("SELECT * FROM SampleTable WHERE donorPhone=?", (str(searchField),))

...
def AddFridge_Window():
    ...
    tk.Button(window_AddFridge, text='Print to Console',
              command=console_PrintFridge, font=myFont).grid(row=7, column=1, sticky="ew")
...
def DeleteFridge_Window():
    ...
    tk.Label(window_DeleteFridge,
             text="Delete fridge with FridgeID: ", font=myFont).grid(row=0)
    fridgeID = tk.Entry(window_DeleteFridge)
    fridgeID.grid(row=0, column=1)

    tk.Button(window_DeleteFridge, text='Delete Fridge',
              command=DeleteFridge, font=myFont).grid(row=5, column=1)
    tk.Button(window_DeleteFridge, text='Back to Fridge Menu',
              command=Open_MainFridge_Window, font=myFont).grid(row=10, column=1)
...
def MainFridge_Window():
    ...
    tk.Button(window_MainFridge, text='Add Fridge',
              command=Open_AddFridge_Window, font=myFont).grid(row=0, column=1, sticky="ew")
    tk.Button(window_MainFridge, text='Delete Fridge',
              command=Open_DeleteFridge_Window, font=myFont).grid(row=2, column=1, sticky="ew")
    tk.Button(window_MainFridge, text='Search Fridge',
              command=Open_SearchFridge_Window, font=myFont).grid(row=4, column=1, sticky="ew")

    tk.Button(window_MainFridge, text='Back to Main Menu',
              command=Open_MainMenu_Window, font=myFont).grid(row=6, column=1, sticky="ew")

    tk.Label(window_MainFridge, height = 1, width = 6, bg="cadet blue").grid(row =1, column =0)
    tk.Label(window_MainFridge, height = 1, width = 6, bg="cadet blue").grid(row =3, column =0)
    tk.Label(window_MainFridge, height = 1, width = 6, bg="cadet blue").grid(row =5, column =0) 
